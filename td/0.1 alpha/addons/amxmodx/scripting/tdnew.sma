...
public cmdmenuPlayerSkill(id) {	
	...
	for(new i ; i < MAX_LEVEL-1 ; i++) {
		formatex(szSkill, 95, "%s \y[POZIOM %d]", gszSkills[i], i+1) 
...
public PlayerSpawn(id){
	...
	if(get_playersnum() == 1 && !gOnePlayerMode && get_pcvar_num(gCvarInfo[CVAR_ONE_PLAYER_MODE])) {
		client_print(id, 3, "%s Znajdujesz sie sam na serwerze!", gszPrefix)
	...
	if(get_playersnum() >= 2 && !gOnePlayerMode && !gWaveIsStarted && !gGameIsStarted)
	{
		client_cmd(0, "spk sound/%s", gSounds[SOUND_CLEAR_WAVE]);
		set_task(5.0, "StartWave", 9123)
	}
...
public cmdOnePlayerMode(id) {
	if(!is_user_alive(id)) {
		...
		return PLUGIN_CONTINUE
	} else if(get_playersnum() > 1 || !gGame) {
