...
public menuBuyTurret2H(id, menu, item)
{
	...
	if(getClosestTurret(ent, 60.0, 1)) {
		client_print(id, print_center,"You can't place turret near other turret!")
		menuBuyTurretH(id, 0, Type-1)
		return PLUGIN_CONTINUE
	}
	else if(find_sphere_class(ent, "func_illusionary", 10.0, entlist, 2) || !fm_is_ent_visible(id, ent)) {
		client_print(id, print_center, "You can't place turret here!")
		menuBuyTurretH(id, 0, Type-1)
		return PLUGIN_CONTINUE
	}
	else if(isTurretInPlacWhereTurretIs(ent, 60.0)) {
		client_print(id, print_center, "You can't place turret here. Wait for other to move turret from here");
		menuBuyTurretH(id, 0, Type-1)
		return PLUGIN_CONTINUE		
	}
...
public menuTurretH(id, menu, item) {
	...
	if(item == 0)
	{
		...
		TurretRangerTurnOff(g_PlayerTurretEnt[id][turret])
		if(g_PlayerTurretAmmo[id][turret] > g_PlayerAlarmValue[id]) {
...
public TurretThink(ent) {
	...
	*/
	...
	if(iShoot && is_valid_ent(iTarget)) 
	{
		...
		if(ammo == g_PlayerAlarmValue[iPlayer])
		{
			new iRet;
...
public LoadTurretsConfig(szMapName[])
{
	...
	if(!file_exists(szDir)) {
		...
		formatex(szDir, charsmax(szDir), g_szTurretsDefaultConfigFile)
...
public _get_alarm_value(index)
	return g_PlayerAlarmValue[index]
...
stock isTurretInPlacWhereTurretIs(iEnt, Float:fRange)
{
	...
	{
		for(new j = 0; j < MAX_PLAYER_TURRET; j++)
		{
			new iTurret = g_PlayerTurretEnt[i][j];
			
			if(iTurret && iTurret != iEnt)
			{
				entity_get_vector(iTurret, EV_VEC_origin, fTempOrigin)
				
				if(abs(fOrigin[0] - fTempOrigin[0]) <= fRange
				|| abs(fOrigin[1] - fTempOrigin[1]) <= fRange
				|| abs(fOrigin[2] - fTempOrigin[2]) <= fRange)
					return iTurret;
		}
	}
